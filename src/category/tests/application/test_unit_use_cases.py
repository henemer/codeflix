...
from __seedwork.application.dto import PaginationOutput, SearchInput, PaginationOutputMapper
...
from category.infra.repositories import CategoryInMemoryRepository, CategoryRepository
...
class TestCreateCategoryUseCaseUnit(unittest.TestCase):
    ...
    def test_if_is_instance_use_case(self):
    ...
    def test_execute(self):
        with patch.object(self.category_repo, 'insert', wraps=self.category_repo.insert) as spy_repo:
...
class TestGetCategoryUseCaseUnit(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        with patch.object(self.category_repo, 'find_by_id', wraps=self.category_repo.find_by_id) as spy_repo:
...
class TestUpdateCategoryUseCase(unittest.TestCase):
    ...
    def test_input(self):
        ...
        description_field = UpdateCategoryUseCase.Input.__dataclass_fields__['description']
    ...
    def test_throw_exception_when_category_not_found(self):
        request = UpdateCategoryUseCase.Input(id='not found', name='test')
        with self.assertRaises(NotFoundException) as assert_error:
            self.use_case.execute(request)
        self.assertEqual(assert_error.exception.args[0], "Entity not found using ID 'not found'")
...
class TestDeleteCategoryUseCase(unittest.TestCase):
    ...
    def test_throw_exception_when_category_not_found(self):
        request = DeleteCategoryUseCase.Input(id='not found')
        with self.assertRaises(NotFoundException) as assert_error:
            self.use_case.execute(request)
        self.assertEqual(assert_error.exception.args[0], "Entity not found using ID 'not found'")
