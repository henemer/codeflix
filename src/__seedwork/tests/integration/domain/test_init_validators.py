...
class TestDRFValidatorIntegration(unittest.TestCase):
    def test_validation_with_error(self):
        ...
        self.assertDictEqual(validator.errors, {
            'name': ['This field is required.'],
            'price': ['This field is required.']})

    def test_validation_without_error(self):
        validator = DRFValidator()
        serializer = StubSerializer(data={'name': 'test', 'price': 5})
        is_valid = validator.validate(serializer)
        self.assertTrue(is_valid)
        self.assertDictEqual(validator.validated_data, {
            'name': 'test',
            'price': 5})
...
class TestStrictBooleanFieldUnit(unittest.TestCase):
    def test_if_is_invalid_when_not_bool_value(self):
