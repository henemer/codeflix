...
class TestInMemoryRepository(unittest.TestCase):
    ...
    def test_throw_exception_not_found_in_find_by_id(self):
        ...
        unique_entity_id = UniqueEntityId(
            'c1f40ced-dfdf-4997-a1ae-84857c6101fd')
        with self.assertRaises(NotFoundException) as assert_error:
...
class TestSearchableRepositoryInterface(unittest.TestCase):
    ...
    def test_throw_error_when_methods_not_implemented(self):
        ...
        self.assertEqual("Can't instantiate abstract class SearchableRepositoryInterface " +
                         "with abstract methods delete, find_all, find_by_id, insert, search, update", assert_error.exception.args[0])
...
class TestSearchParams(unittest.TestCase):
    ...
    def test_props_annotations(self):
        self.assertEqual(SearchParams.__annotations__, {'page': Optional[int],
...
class TestSearchResult(unittest.TestCase):
    ...
    def test_when_per_page_is_greather_than_total(self):
        result = SearchResult(
            items=[],
            total=4,
            current_page=1,
...
class TestInMemorySearchableRepository(unittest.TestCase):
    ...
    def test_search_applying_sort_and_paginate(self):
        ...
        for index, item in enumerate(arrange_by_asc):
            ...
            self.assertEqual(
                result, item['output'], f"The output using sort_dir asc on index {index} is differnt")
        ...
        for index, item in enumerate(arrange_by_desc):
            result = self.repo.search(item['input'])
            self.assertEqual(
                result, item['output'], f"The output using sort_dir desc on index {index} is differnt")
